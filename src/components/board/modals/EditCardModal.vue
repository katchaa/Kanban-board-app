<template>
	<Teleport to="body">
		<div
			class="flex justify-center items-start w-screen h-screen absolute left-0 top-0 pt-36 z-50 bg-black/40"
			v-if="props.show"
		>
			<section class="bg-gray-100 w-96 shrink-0 p-2 rounded-md shadow-md">
				<h1 class="text-lg font-semibold ml-2">Edit card</h1>
				<form
					@submit.prevent="editCard()"
...
const editCard = async () => {
	await projectStore.editCard(props.card.id, title.value)
